...
function GfxBoard(props) {
  ...
  useEffect(()=>{
    if (f1_IsPressed) {
      ...
      tcpStrings.freeThrows.in().map((string, iter)=>{send(string)})
      tcpStrings.freeThrows.main(
        getLastPlayerData(2),
        getAbr(),
        getLastPlayerData(3),
      ...
      props.setEscCommand(tcpStrings.freeThrows.out())
    }
  }, [f1_IsPressed])
  ...
  useEffect(()=>{
    if (f2_IsPressed) {
      ...
      tcpStrings.twoPoints.in().map((string, iter)=>{send(string)})
      tcpStrings.twoPoints.main(
        getLastPlayerData(2),
        getAbr(),
        getLastPlayerData(3),
      ...
      props.setEscCommand(tcpStrings.twoPoints.out())
      }}, [f2_IsPressed])

  useEffect(()=>{
    if (f3_IsPressed) {
      console.log("F3 pressed")
      tcpStrings.threePoints.in().map((string, iter)=>{send(string)})
      tcpStrings.threePoints.main(
        getLastPlayerData(2),
        getAbr(),
        getLastPlayerData(3),
      ...
      props.setEscCommand(tcpStrings.threePoints.out())
    }
  }, [f3_IsPressed])
    ...
    <Container>
      <Column>
        ...
        <ComButton
          style={{width: '100%'}}
          text={'Free throws (F1)'}
          messageIn={tcpStrings.freeThrows.in()}
          messageMain={tcpStrings.freeThrows.main(
            getLastPlayerData(2),
            getAbr(),
            getLastPlayerData(3),
            {scored: getLastPlayerData(9), 
          ...
          onClick={()=>{props.setEscCommand(tcpStrings.freeThrows.out())}}
        />
        <ComButton
          style={{width: '100%'}}
          text={'2 points (F2)'}
          messageIn={tcpStrings.twoPoints.in()}
          messageMain={tcpStrings.twoPoints.main(
            getLastPlayerData(2),
            getAbr(),
            getLastPlayerData(3),
            {scored: getLastPlayerData(11), 
          ...
          onClick={()=>{props.setEscCommand(tcpStrings.twoPoints.out())}}
        />
        <ComButton
          style={{width: '100%'}}
          text={'3 points (F3)'}
          messageIn={tcpStrings.threePoints.in()}
          messageMain={tcpStrings.threePoints.main(
            getLastPlayerData(2),
            getAbr(),
            getLastPlayerData(3),
            {scored: getLastPlayerData(13), 
          ...
          onClick={()=>{props.setEscCommand(tcpStrings.threePoints.out())}}
        />
      </Column>
      <Column>
        <Title>PODPISI</Title>
